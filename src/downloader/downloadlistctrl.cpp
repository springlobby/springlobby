...
DownloadListCtrl::DownloadListCtrl(wxWindow* parent)
    : DownloadListCtrl::BaseType(parent, TLIST_CLICK, wxDefaultPosition, wxDefaultSize,
				 wxSUNKEN_BORDER | wxLC_REPORT | wxLC_SINGLE_SEL | wxLC_ALIGN_LEFT, _T("DownloadListCtrl"), 3, &DownloadListCtrl::CompareOneCrit)
    , m_popup(0)
{
...
int DownloadListCtrl::CompareOneCrit(DataType u1, DataType u2, int col, int dir) const
{
	switch (col) {
		case 0:
			return dir * compareSimple(strtolower(std::string(u1.name.mb_str())), strtolower(std::string(u2.name.mb_str())));
...
wxString DownloadListCtrl::GetItemText(long item, long column) const
{
	...
	switch (column) {
		...
		case 2:
			return wxString::Format(wxT("%i%%"), (int)((double)100.0 * infos.progress / (double)infos.size)); //(wxString::Format(_T("%.2f") ) % ( infos.progress * 100 )).str() : na_str;
		case 3:
			return wxEmptyString; //infos.inspeed > -0.01 ? (wxString::Format(_T("%.2f") ) % ( infos.inspeed*kfactor )).str() : na_str;
		case 4:
			return wxEmptyString; //infos.inspeed > -0.01 ? (wxString::Format(_T("%.2f") ) % ( infos.inspeed*kfactor )).str() : na_str;
		case 5:
