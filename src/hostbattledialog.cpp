...
HostBattleDialog::HostBattleDialog( wxWindow* parent ): wxDialog( parent, -1, _("Host new battle"), wxDefaultPosition, wxSize( 340,300 ), wxDEFAULT_DIALOG_STYLE )
{
  ...
  m_desc_text = new wxTextCtrl( this, wxID_ANY, WX_STRING(sett().GetLastHostDescription()), wxDefaultPosition, wxDefaultSize, 0 );
	...
	wxBoxSizer* m_rank_sizer;
	m_rank_sizer = new wxBoxSizer( wxHORIZONTAL );

    m_rank_lbl = new wxStaticText( this, wxID_ANY, _("Required rank"), wxDefaultPosition, wxDefaultSize, 0 );
	m_rank_sizer->Add( m_rank_lbl, 1, wxALL, 5 );

	m_rank_pic = new wxChoice( this, wxID_ANY, wxDefaultPosition, wxDefaultSize );
	m_rank_pic->SetToolTip( _("The minimum rank that can join this game.") );

	m_rank_sizer->Add( m_rank_pic, 2, wxALL, 5 );

	m_main_sizer->Add( m_rank_sizer, 0, wxEXPAND, 5 );
...
void HostBattleDialog::FillRanks()
{
    m_rank_pic->Insert( _("None") , 0);
    m_rank_pic->Insert( User::GetRankName(RANK_1), 1 );
    m_rank_pic->Insert( User::GetRankName(RANK_2), 2 );
    m_rank_pic->Insert( User::GetRankName(RANK_3), 3 );
    m_rank_pic->Insert( User::GetRankName(RANK_4), 4 );
    m_rank_pic->Insert( User::GetRankName(RANK_5), 5 );
    m_rank_pic->Insert( User::GetRankName(RANK_6), 6 );
    int last = sett().GetLastRankLimit();
    if (last != 0) m_rank_pic->SetSelection( last );
}
...
void HostBattleDialog::OnOk( wxCommandEvent& event )
{
  ...
  sett().SetLastRankLimit( (m_rank_pic->GetSelection())  * 100 );
  sett().SaveSettings();
  EndModal( wxID_OK );
}
