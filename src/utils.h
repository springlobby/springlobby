...
#define ASSERT_LOGIC(cond,msg) if(!(cond)){wxLogFatalError(_T("logic ")+ wxString(msg, wxConvUTF8 ));throw std::logic_error(msg);}
#define ASSERT_RUNTIME(cond,msg) if(!(cond)){wxLogDebug(_T("runtime ")+ wxString(msg, wxConvUTF8 ));throw std::runtime_error(msg);}
