...
int wxKeyBind::StringToKeyCode(const wxString& keyName)
{
	...
	if (keyName == wxT("ALT"))
		return WXK_ALT;
	if (keyName == wxT("SHIFT"))
		return WXK_SHIFT;

	if (keyName == wxT("PAUSE"))
		return WXK_PAUSE;
	if (keyName == wxT("PRINT"))
		return WXK_PRINT;
	...
	if (keyName == wxT("0 (numpad)"))
		return WXK_NUMPAD0;
	if (keyName == wxT("1 (numpad)"))
		return WXK_NUMPAD1;
	if (keyName == wxT("2 (numpad)"))
		return WXK_NUMPAD2;
	if (keyName == wxT("3 (numpad)"))
		return WXK_NUMPAD3;
	if (keyName == wxT("4 (numpad)"))
		return WXK_NUMPAD4;
	if (keyName == wxT("5 (numpad)"))
		return WXK_NUMPAD5;
	if (keyName == wxT("6 (numpad)"))
		return WXK_NUMPAD6;
	if (keyName == wxT("7 (numpad)"))
		return WXK_NUMPAD7;
	if (keyName == wxT("8 (numpad)"))
		return WXK_NUMPAD8;
	if (keyName == wxT("9 (numpad)"))
		return WXK_NUMPAD9;
	if (keyName == wxT("= (numpad)"))
		return WXK_NUMPAD_EQUAL;
	if (keyName == wxT("ENTER (numpad)"))
		return WXK_NUMPAD_ENTER;

	if (keyName == wxT("JOYLEFT"))
		return WXK_JOYLEFT;
	if (keyName == wxT("JOYRIGHT"))
		return WXK_JOYRIGHT;
	if (keyName == wxT("JOYUP"))
		return WXK_JOYUP;
	if (keyName == wxT("JOYDOWN"))
		return WXK_JOYDOWN;
	if (keyName == wxT("JOYW"))
		return WXK_JOYW;
	if (keyName == wxT("JOYX"))
...
bool wxCmd::Save(wxConfigBase* p, const wxString& key, bool bCleanOld) const
{
	...
	wxString value = wxString::Format(_T("%s|%s|%s"), STD_STRING(GetName()).c_str(), STD_STRING(GetDescription()).c_str(), STD_STRING(shortcuts).c_str());
	...
	if (p == NULL) {
		...
		if (client == NULL) {
			...
			wxKBLogDebug(wxT("wxKeyBinder::OnChar - ignoring this keyevent [%d] because I'm not ")
				     wxT("attached to the window which generated the keypress..."),
				     event.GetKeyCode());
			event.Skip(); // ... skip it
			return;
		}
...
bool wxKeyBinder::Save(wxConfigBase* cfg, const wxString& key, bool bCleanOld) const
{
	...
	for (size_t i = 0; i < m_arrCmd.GetCount(); i++) {
		...
		wxString keyname = wxString::Format(_T("%s%s%d-type%d"), STD_STRING(basekey).c_str(), wxCMD_CONFIG_PREFIX, curr->GetId(), curr->GetType());
