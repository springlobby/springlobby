...
hotkey_parser::hotkey_parser(const wxString& uikeys_filename)
    : m_dontTouch(true)
{
...
bool hotkey_parser::processLine(const wxString& line)
{
	...
	if (tokLine.size() == 1) { //unbindall?
		...
		return true;
	} else if (tokLine.size() == 2) { //fakemeta=
...
void hotkey_parser::writeBindingsToFile(const key_binding& springbindings)
{
	...
	//backup our current uikeys.txt
	{
		if (wxRenameFile(this->m_filename, prevFilenameBak) == false) {
			throw HotkeyException(wxString::Format(_("Error renaming %s to %s"), m_filename.c_str(), prevFilenameBak.c_str()));
		}
	}
