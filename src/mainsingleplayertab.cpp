...
m_ui(ui)
{
  ...
  #ifdef HAVE_WX26
  m_tabs->InsertPage( 1, m_opts_tab, _("Unit Restrictions"), false, 1 );
  #else
  m_tabs->InsertPage( 1, m_opts_tab, _("Unit Restrictions"), false, wxIcon(battle_settings_xpm) );
  #endif
  m_mm_opts_tab = new BattleroomMMOptionsTab( m_sp_tab->GetBattle(), m_tabs);
  #ifdef HAVE_WX26
  m_tabs->InsertPage( 2, m_mm_opts_tab, _("Options"), false, 1 );
  #else
