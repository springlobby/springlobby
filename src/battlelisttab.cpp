...
void BattleListTab::OnHost( wxCommandEvent& event )
{
  if ( !m_ui.IsConnected() ) {
    ...
    customMessageBox(SL_MAIN_WINDOW_PTR, m_sl_icon, _("You cannot host a game while being offline. Please connect to a lobby server."), _("Not Online."), wxOK );
    m_ui.ShowConnectWindow();
    return;
  }
  if ( !m_ui.IsSpringCompatible() ){
    wxLogWarning(_T("Hosting is disabled due to the incompatible version ") );
    customMessageBox(SL_MAIN_WINDOW_PTR, m_sl_icon,_("Hosting is disabled due to the incompatible version you're using"), _("Spring error"), wxICON_EXCLAMATION);
    return;
  }
  if ( m_ui.IsSpringRunning() ) {
    wxLogWarning(_T("trying to host while spring is running") );
    customMessageBox(SL_MAIN_WINDOW_PTR, m_sl_icon,_("You already are running a Spring instance, close it first in order to be able to host a new game"), _("Spring error"), wxICON_EXCLAMATION );
    return;
  }
  ...
  if ( dlg.ShowModal() == wxID_OK ) {
    ...
    try {
      ...
      customMessageBox(SL_MAIN_WINDOW_PTR,  m_sl_icon,_("Battle not started beacuse the mod you selected could not be found. "), _("Error starting battle."), wxOK );
      return;
    }
    ...
    try {
      if ( mname != "" ) map = usync()->GetMap( mname );
        ...
        customMessageBox(SL_MAIN_WINDOW_PTR, m_sl_icon, _("Couldn't find any maps in you spring installation. This could happen when you set the Spring settings incorrectly."), _("No maps found"), wxOK );
        return;
      } else {
        map = usync()->GetMap( 0 );
      }
...
void BattleListTab::DoJoin( Battle& battle )
{
  if ( !m_ui.IsSpringCompatible() ){
    ...
    customMessageBox(SL_MAIN_WINDOW_PTR, m_sl_icon,_("Joining battles is disabled due to the incompatible spring version you're using."), _("Spring error"), wxICON_EXCLAMATION);
    return;
  }
  ...
  if ( m_ui.IsSpringRunning() ) {
    ...
    customMessageBox(SL_MAIN_WINDOW_PTR, m_sl_icon,_("You already are running a Spring instance, close it first in order to be able to join another battle."), _("Spring error"), wxICON_EXCLAMATION );
    return;
  }

  if ( !battle.ModExists() ) {
    if (customMessageBox(SL_MAIN_WINDOW_PTR,  m_sl_icon,_("You need to download the mod before you can join this game.\n\nDo you want me to take you to the download page?"), _("Mod not awailable"), wxYES_NO | wxICON_QUESTION ) == wxYES ) {
  ...
  if ( !battle.MapExists() ) {
    if (customMessageBox(SL_MAIN_WINDOW_PTR, m_sl_icon, _("You need to download the map to be able to play in this game.\n\nDo you want me to take you to the download page?"), _("Map not awailable"), wxYES_NO | wxICON_QUESTION ) == wxYES ) {
