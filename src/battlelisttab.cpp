...
void BattleListTab::AddBattle( Battle& battle ) {
 ...
 // ASSERT_LOGIC( m_battle_list->GetItem( item ), _T("!GetItem") );
  ...
  m_battle_list->SetItem( index, 4, RefineMapname( battle.GetMapName() ), battle.MapExists()?IconImageList().ICON_EXISTS:IconImageList().ICON_NEXISTS );
  m_battle_list->SetItem( index, 5, RefineModname( battle.GetModName() ), battle.ModExists()?IconImageList().ICON_EXISTS:IconImageList().ICON_NEXISTS );
